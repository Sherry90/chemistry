...
6. 반응 시뮬레이션: 반응물 + 조건 → 생성물 예측 + 간단한 결합 변화 애니메이션
...
5. **렌더 모드는 Ball-and-stick 단일 구현으로 시작**. 단, 타입/인터페이스는 `RenderMode` 유니온으로 설계하여 Space-filling / Wireframe 등 다른 모드를 **중단 없이 확장**할 수 있어야 한다. 초기 버전에서 다른 모드의 실제 렌더 구현은 제공하지 않는다 (토글 UI는 "준비 중" 비활성화 가능).
...
- 결합 차수(단/이중/삼중)는 **기본 실린더 다중 표현**(2중: 평행 실린더 2개, 3중: 3개). 차후 대체 표현은 타입만 확장 여지.
