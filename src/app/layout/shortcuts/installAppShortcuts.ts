...
import { installGlobalUndoShortcuts, type KeyActionId } from '@/viewport';
import type { UndoableDispatcher } from '@/stores';

export interface InstallAppShortcutsOpts {
  readonly dispatcher: UndoableDispatcher;
  /** Phase 11 (D-SHORTCUT-OPEN-STATE) — KEY_MAP 비-undo global 바인딩
   *  (showShortcutSheet 등) 단일 디스패치 통지. App.tsx 가 주입. */
 ...
 */
export function installAppShortcuts({ dispatcher, onAction }: InstallAppShortcutsOpts): () => void {
  const undoCleanup = installGlobalUndoShortcuts({
    dispatcher,
    ...(onAction ? { onAction } : {}),
  });
  return () => {
