...
- 모든 화합물의 SMILES 가 Phase 03 `parseSmiles` 로 파싱되고, 모든 3D 좌표가 Phase 03 `RdkitBackend.embed` 또는 **PubChem 3D SDF** 중 하나의 결정적 경로로 확보된다.
...
- **정규화** (`scripts/pubchem-fetch/normalize.ts` 는 `src/engine/pubchem/normalize.ts` 의 thin wrapper):
  ...
  - **공용 모듈 강제**: 본 Phase 의 빌드 스크립트와 phase-05 런타임 normalize 가 _동일_ `src/engine/pubchem/normalize.ts` 를 import 한다 (phase-05 §6.8 D1). 양쪽이 `NORMALIZE_SCHEMA_VERSION` 상수를 공유하므로 한쪽만 bump 되어 데이터-캐시 정합성이 깨질 위험이 없다.
...
- **검증 파이프라인** (`scripts/pubchem-fetch/validate.ts`):
  ...
  - PubChem 3D SDF 가 없는 경우 Phase 03 `RdkitBackend.embed` (고정 시드) 로 생성
  - 임베드 실패 시 화합물 드롭 + 리포트
...
- **Phase 01** — `Compound`/`CompoundId`/`CompoundProvenance`, `Molecule`, `Atom`, `Bond`, `BondOrder`, 식별 모듈 `@/chemistry/compounds/ids` (`SerializedMolecule`, `indexToId`, `createAtomId`/`createBondId`), `Result`, logger
...
| D5  | **3D 좌표 소스 우선순위** | **1순위: PubChem 3D SDF** (있으면), **2순위: Phase 03 `RdkitBackend.embed` 고정 시드**. 동일 청크에 두 소스 혼재 가능. 엔트리에 `coordinateSource: 'pubchem-3d' \| 'rdkit-etkdg'` 기록                                                                            |
...
export interface Compound {
  readonly cid: CompoundId | null; // Phase 01 (brand)
   ...
   * PubChem 의 `CanonicalSMILES` 가 아니라 `IsomericSMILES` 를 입력으로 받아
   * `RdkitBackend.toCanonical().smiles` 를 거친 결과. R/S·E/Z 정보가 SMILES 문자열에 보존된다. */
  readonly smiles: string; // Phase 01
  /** PubChem 응답의 `InChI` 필드를 **우선 사용**. 응답 누락(드물게 발생) 시
   * `RdkitBackend.toCanonical().inchi` 로 역산하여 채운다. 두 경로 모두 동일 RDKit 버전이므로
   * phase-05 런타임이 같은 SMILES 를 정규화했을 때와 byte-identical. */
  readonly inchi: string | null; // Phase 01
  readonly defaultMolecule: Molecule | null; // Phase 01 (3D 좌표 포함)
  readonly provenance: CompoundProvenance; // Phase 01 — 청크 엔트리는 항상 'manifest'

  // Phase 04 추가
  /** PubChem 응답의 `InChIKey` 필드를 **우선 사용**. 응답 누락 시 `RdkitBackend.toCanonical().inchiKey`
...
> Phase 01 의 `Compound.smiles` 는 "SMILES" 로만 명시되어 있었으나, 본 Phase 는 **RDKit canonical SMILES (입체화학 보존)** 로 의미를 좁힌다. 구체적으로: PubChem 의 `IsomericSMILES` 를 입력 → `RdkitBackend.parseSmiles` (RDKit 인스턴스는 Node 용) → `RdkitBackend.toCanonical().smiles` 의 결과 문자열. 이는 범위 확장이 아닌 **의미 확정**이며, 하위 호환(선행 구현 없음) 이므로 타입 파일의 주석으로 명시. PubChem `CanonicalSMILES` 는 입체 정보를 제거하므로 **입력 후보에서 제외** (§6.5).
>
> **불변식 적용 범위**: 이 "RDKit canonical, stereo-preserving" 제약은 **본 Phase 가 커밋하는 청크 엔트리에만** 성립한다. 사용자가 스토어 레이어(Phase 07) 에서 런타임에 `cid: null` / `defaultMolecule: null` 상태로 임시 구성한 `Compound` 에는 이 불변식이 아직 성립하지 않으며, 스토어는 편집이 확정될 때 `RdkitBackend.toCanonical` 을 통과시켜 불변식을 복원한다 (Phase 07 책임). 즉 **타입 수준에서 강제되는 제약이 아니라 "데이터셋 레벨 불변식"**.
...
**직렬화 형태는 Phase 01 `SerializedMolecule`** (`@/chemistry/compounds/ids`): 원자/결합에 brand ID 가 **없고** 정수 인덱스만 사용한다. `aAtomIndex`/`bAtomIndex` 는 `atoms[]` 배열 인덱스다. `order` 는 `BondOrder` 리터럴 — 방향족 결합은 숫자가 아닌 문자열 `"aromatic"` 으로 직렬화한다. 예: 벤젠 고리 결합

```json
"bonds": [
  { "aAtomIndex": 0, "bAtomIndex": 1, "order": "aromatic" },
  { "aAtomIndex": 1, "bAtomIndex": 2, "order": "aromatic" }
]
```
...
- `defaultMolecule` 은 직렬화 형태(`SerializedMolecule`)이며 런타임 `Molecule` 과 **1:1 이 아니다**. 런타임 로더(`loadCompoundChunk`, §6.9)가 Phase 01 `indexToId(raw)` 코덱으로 복원하면서: ① 각 atom 에 `createAtomId()`, 각 bond 에 `createBondId()` 를 부여하고 ② `aAtomIndex`/`bAtomIndex` 를 해당 atom 의 새 `AtomId` 로 치환하며 ③ `Molecule.id` 를 **`cid` 에서 결정적으로 파생한 `MoleculeId`** (`moleculeIdForCid(cid)`, Phase 07 §\_shared) 로 설정한다. 동일 CID 재로드 시 `Molecule.id` 는 동일(중복 식별 가능), atom/bond brand ID 는 매 로드 새로 생성(런타임 안정 ID 규약, CD1).
- 직렬화 JSON 에는 `defaultMolecule.id` 필드를 **두지 않는다** (런타임 로더가 `cid` 로부터 파생). 사용자 생성 분자의 UUID 와는 파생 규칙으로 구분된다.
...
```json
{
  ...
  "categories": {
    ...
    "biomolecules-basic": {
      "chunks": ["biomolecules-basic.001.json", "biomolecules-basic.002.json"],
      "count": 298
    }
  },
  "entries": [
...
```json
{
  ...
  "dropped": [
    { "cid": 123456, "reason": "SmilesSyntax", "detail": "..." },
    {
      "cid": 789012,
      "reason": "EmbedFailed",
      "detail": "ETKDG 3 retries failed (primary + 2 random seeds)"
    }
  ],
  "seedFiles": [{ "path": "seeds/inorganic-common.tsv", "entries": 80 }],
...
export const FORMULA_MAP_GENERATED: ReadonlyMap<
  string,
  { smiles: string; cid: number; priority: number }
> = new Map([
...
- **생성 규칙 (결정적)**: 같은 Hill 키를 공유하는 후보들 중 **`(priority ASC, cid ASC)`** 로 정렬한 _첫 번째_ 1개를 선택. 즉 priority 가 가장 낮은 것을 먼저, priority 동률이면 CID 가 가장 작은 것. **동일 빌드 입력 (시드 + 캐시) → 항상 동일 출력** 을 보장한다 (§10 #11 byte-identical 재현성과 일관). `name.en` 등 다른 키는 결정에 영향을 주지 않는다 (cid 가 이미 unique 해 tie-break 가 종결되므로). 향후 추가 정렬 키는 본 §4.8 + §6.8 동시 갱신 필요.
...
[seeds/*.tsv]
 ...
 normalize()                       ── PubChem → Compound (properties 매핑)
     │
     ▼
 validateAndEmbed()                ── Phase 03 parseSmiles + (PubChem 3D ?? RdkitBackend.embed)
     │  (병렬도 제한, 메모리 상한 고려)
     ▼
...
- **SMILES 정규화 파이프라인**: `IsomericSMILES` → `RdkitBackend.parseSmiles` → `RdkitBackend.toCanonical` (phase-03 §6.8, async) 의 `.smiles` 로 **RDKit 캐노니컬** 재산출. PubChem 의 캐노니컬 규약은 RDKit 과 토큰 순서가 다를 수 있으므로 저장 SMILES 는 RDKit canonical 로 통일. `RdkitBackend.toCanonical` 의 SMILES 는 **입력에 입체 정보가 있을 때 이를 보존**하므로 정규화 후에도 R/S·E/Z 유지.
- **InChI / InChIKey 정규화 파이프라인**: PubChem 응답의 `InChI` / `InChIKey` 필드를 **우선 사용**. 두 필드 중 하나라도 누락되면 (드물게 발생) `RdkitBackend.toCanonical` 의 `.inchi`/`.inchiKey` 로 역산하여 양쪽 모두 채운다. 빌드타임과 런타임이 동일 RDKit 버전 (`compounds.meta.json.rdkitVersion`) 을 공유하므로, phase-05 가 동일 CID 를 PubChem 에서 다시 받아 정규화해도 _byte-identical_ InChIKey 가 산출된다 (캐시 역조회 정합성의 핵심). RDKit 버전 업그레이드 시 InChIKey 가 변할 수 있으나, 이는 `rdkitVersion` 갱신 + 데이터 재수집을 강제하는 PR 단위 변화로 흡수.
...
- **3D 좌표 소스 (D5)**:
  ...
  2. 없으면: `RdkitBackend.embed(parsedMol, resolveEmbedOptions())` — `DEFAULT_EMBED_OPTIONS.seed = EMBED_SEED_PRIMARY` (phase-03 §6.5 상수 import). `coordinateSource: 'rdkit-etkdg'`.
  3. **재임베드 정책 (D9 — 완전 결정적)**:
     - **시도 1**: `seed = EMBED_SEED_PRIMARY` (= `0xC0FFEE`).
     - **시도 2~3**: `phase-03 §6.5 EMBED_RETRY_SEEDS` (= `[PRIMARY+1, PRIMARY+2]`) 를 순서대로. `useRandomCoords: false` 유지 (랜덤 시드 사용 안 함).
     - 모든 시드 소진 후 `EmbedFailed` → **해당 화합물 드롭** + `compounds.meta.json.dropped[]` 에 `{ cid, reason: 'EmbedFailed', detail: 'ETKDG failed for all fixed seeds (PRIMARY, +1, +2)' }` 기록. 시드 파일에서는 제거하지 않음.
     - **결정성**: 모든 시도가 고정 시드 시퀀스이므로 성공·실패·최종 좌표 모두 **완전히 결정적**이다 (재시도로 성공한 경우 포함). §10 #11 byte-identical 보장은 분기 제한 없이 성립.
...
└── pubchem-fetch/
    ...
    ├── validate.ts                   # parseSmiles / RdkitBackend.embed 통합
    ├── chunk.ts                      # 청크/샤드 분할
...
| R3  | PubChem 3D SDF 미제공 화합물 비율이 예상보다 높음                                              | 중   | Phase 03 `RdkitBackend.embed` 로 대체. 대부분의 교과 화합물은 3D 제공됨을 샘플 체크 (Phase 04 착수 시 20–50종 사전 조사) |
...
6. **SMILES 라운드트립 검증 (CI 기본: 샘플링)**: 결정적 시드 기반 샘플 **200개의 `smiles`** 가 Phase 03 `parseSmiles` 로 `Result.ok`; 그중 **샘플 20개**가 `RdkitBackend.embed` 성공 및 좌표 재현성(RMSD < 1e-6) 확인. 전수 검증 여부는 §11 Q7 에서 결정하며, 본 기본안은 "샘플링 채택". 기본안이 변경되면 본 항목도 "전수 2487개 모두 `parseSmiles` 통과" 로 갱신.
...
13. **Schema 동기 검증**: 빌드 산출물(`compounds.meta.json.normalizeSchemaVersion`) 의 값이 빌드 시점 `src/engine/pubchem/normalize.ts` 의 `NORMALIZE_SCHEMA_VERSION` 상수와 정확히 일치한다. CI step `scripts/check-schema-version.ts` 가 (a) 빌드 스크립트가 실제로 _공용_ `src/engine/pubchem/normalize.ts` 를 import 하는지 (`scripts/pubchem-fetch/normalize.ts` 가 thin wrapper 임을 정적 검증), (b) 매니페스트 메타와 상수 값이 일치하는지 검증한다. 한쪽만 bump 되면 CI 실패. 이 검증은 phase-05 §10 (DoD) 의 schemaVersion-guard 와 짝을 이룬다.
