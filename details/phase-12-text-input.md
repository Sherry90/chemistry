...
9. **공유 helper 분리** — phase-11 §6.12 의 인라인 `withRegeneratedIds` 를 `src/chemistry/compounds/regenerateIds.ts` 로 분리 (phase-11 retrofit 소급) → 본 Phase 의 replace 경로가 동일 헬퍼 import.
...
본 Phase 는 `architecture.md` §12 의 열린 질문 중 **#1 (반응 예측 한계)** / **#2 (PubChem 큐레이션)** / **#3 (ΔH)** / **#5 (WebGL2)** / **#6 (이온)** 모두 _영역 외_. **#4 (RDKit 메인 스레드)** 는 본 Phase 가 phase-03 `RdkitBackend` 인터페이스만 사용 — Worker 분리 결정은 Phase 14 가 측정.
...
- `phase === 'error'` 시: 본 컴포넌트 자체는 _미표시_ — preview 도 자연스럽게 error 분기 (ParseError.code === 'RdkitNotReady' 또는 RdkitInitError 발생). ErrorMessage 가 `mapIngestErrorToKey` 로 메시지 + `'reload-page'` 액션 표시.

##### `<DuplicateWarning existingId onActivate/>`

- preview success 시점에 본 Phase 의 모달이 _제출 전_ `selectMoleculeIds` + `selectMoleculeById` 로 모든 분자의 InChIKey 와 비교. 매칭이 있으면 본 컴포넌트 마운트.
...
- replace=on 인 경우는 본 컴포넌트 _비표시_ (사용자가 명시적으로 교체 의도).
...
##### `src/chemistry/compounds/regenerateIds.ts` (신규, phase-11 retrofit 소급)

```ts
import type { Molecule } from '@/chemistry/compounds/types';
import type { MoleculeId } from '@/chemistry/compounds/ids';
import { createAtomId, createBondId, createMoleculeId } from '@/chemistry/compounds/ids'; // CD1
...
`MoleculeCardSource` 유니온 = `'compound' | 'reaction-product' | 'text-input'` (additive). 표시 분기는 _상단 출처 라벨_ 만 변경:
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **03 (Chemistry Engine)** | _직접 import 안 함_ (D-PREVIEW-PATH 옵션 A 정합). 모든 phase-03 호출은 `moleculeStore.previewFromText` / `addFromSmiles/Inchi/Formula` 를 통해 store 내부에서. **타입만 import**: `ParseError`, `EmbedError`, `ParsedMol`, `InputKind`, `RdkitStatus` (TextInputMode 별칭 정의용). RDKit 상태 구독은 `onRdkitStatusChange` 도 store 경유 (phase-07 retrofit 가능 — 본 Phase 는 _panel-local hook_ `useRdkitStatus()` 가 phase-03 의 export 를 _직접_ 구독하는 예외 허용 — phase-08 가 viewport/* 에서 chemistry/elements 직접 import 한 P1 명시 예외 패턴과 동일). 단, RDKit 상태 = *읽기 전용 구독\* 이지 phase-03 동작 함수 호출 아님 → P1 정신 위배 아님. **레이어 가드**: §7.3 의 `onRdkitStatusChange` 한정 import 화이트리스트. |
...
- **공용 타입**: `IngestError`, `AsyncState<T,E>`, `MoleculeId`, `Notification`, `PanelKey` (확장). |
  | **08 (Rendering)** | _직접 import 안 함_. UI 컴포넌트는 `<ViewportApi>` 사용 안 함 (모달 내부 = 뷰포트 비조작). (식별 팩토리는 Phase 08 아닌 Phase 01 `@/chemistry/compounds/ids` — CD1.) |
  | **01 (chemistry, CD1)** | `createAtomId` / `createBondId` / `createMoleculeId` (`@/chemistry/compounds/ids`), `withRegeneratedIds` (`@/chemistry/compounds/regenerateIds`) — `regenerateIds` 가 import. |
  | **09 (Interactions)** | _직접 import 안 함_. `KEY_MAP` 변경 없음. phase-09 D10 `isTextInputTarget` 가드는 자동 통과 — 본 Phase 의 모달 안 native `<input>` / `<textarea>` 가 `tagName === 'INPUT'/'TEXTAREA'` 매칭. **검증 의무**: §6.12 + C7 컴포넌트 테스트. |
...
본 Phase 는 `engine/parser` / `engine/geometry` / `services/*` _내부_ 를 직접 import 하지 않는다 (D-PREVIEW-PATH 옵션 A). `chemistry/elements` (PeriodicTable 의 Element 타입만) 와 **`@/chemistry/compounds/ids`** (Phase 01, CD1 — `regenerateIds` 의 `createAtomId`/`createBondId`/`createMoleculeId`) 는 type-level / 순수 함수 import 한정 예외. (`viewport/ids` 아님 — 식별 모델이 Phase 01 chemistry 계층으로 상향됨, CD1.)
...
| **P5**  | 도메인 실패는 Result 패턴으로 store 의 AsyncState / Result 분기에 흘러옴. 본 Phase 의 ErrorMessage 는 _Result.error_ 만 표시. `<PanelErrorBoundary>` 는 _런타임 예외_ (예: ref undefined) 만 포착.                                                                                                  | architecture §3.7 / phase-10 P5 / phase-11 P5 |
| **P6**  | i18n 부팅 실패 안전 — `<AppErrorBoundary>` fallback 정적 영문. 본 Phase 패널은 정상 부팅 가정 + Suspense fallback 만.                                                                                                                                                                               | phase-10 D15 / phase-11 P6                    |
| **P7**  | `experimental` 배지 / disclaimer 는 phase-06 §4.8 가 정의. 본 Phase 는 그 키 _재사용 안 함_ (반응 결과 영역 — 본 Phase 영역 외).                                                                                                                                                                    | phase-06 §4.8                                 |
...
| **P9**  | Radix Dialog 의 focus trap + Esc 자동 닫기 + trigger 복원. 본 Phase 는 _추가 키보드 핸들링 없음_ (Radix 기본).                                                                                                                                                                                      | phase-10 §6.x / phase-11 P9                   |
| **P10** | `panels.active` ↔ `is*Open` 두 슬롯의 의미 분기 — phase-07 가 보장. 본 Phase 의 `'text-input'` modal 은 `panels.active` 의 docked 슬롯과 _동시 표시 가능_.                                                                                                                                          | phase-07 §4.4 / phase-10 P10 / phase-11 P11   |
...
| **D2**                     | 모달 vs docked                                          | **modal** (PanelKey mode='modal'). 사용자 진입점이 _명시적 trigger_ (Toolbar 또는 PeriodicTable 셀) 라 docked 상시 표시는 화면 점유 과다.                                                                                                                                | §2.1               |
...
| **D-RDKIT-STATUS-IMPORT**  | `onRdkitStatusChange` import 경로                       | **panel-local hook 이 phase-03 직접 import** (P1 명시 예외 — read-only 구독 한정). store retrofit (`uiStore.rdkitStatus`) 는 _과잉 설계_ — 모달 닫혀 있을 때 store 슬롯 무의미.                                                                                          | §3.1, §6.10        |
...
| **D-REGENID-MODULE**       | `withRegeneratedIds` export 경로                        | `src/chemistry/compounds/regenerateIds.ts` 신규 모듈. phase-11 §6.12 인라인 → import 변경 (소급).                                                                                                                                                                        | §2.1, §12.1        |
| **D-LOG-RUNTIME**          | 패널 내부 런타임 예외                                   | `<PanelErrorBoundary panelKey="text-input" slotName="modal">` 가 `logger.error('panel-boundary.caught', ...)` 자동. 본 Phase 컴포넌트는 _추가 logger 호출 안 함_ (Boundary 가 책임). 도메인 실패 (Result.error) 는 ErrorMessage 가 표시 (logger 호출 안 함 — 정상 흐름). | §6 전반            |
...
본 Phase 는 새 도메인 타입을 _최소_ 도입한다. 대부분 phase-03 / phase-07 의 기존 타입 재사용.
...
// 비동기 진입점은 parse → embed → commit 을 **액션 내부에서** 수행한다 (별도 public
// `commit()` 단계 없음). architecture.md §7.2 의 흐름은 *예시적* 표기이며
// `commit` 은 `addFromSmiles`/`addFromFormula` 내부 단계임을 arch §7.2 가 명시 (CD4 정합).
...
(phase-11 의 SOURCE*LABEL 매핑이 *형태로만 존재\_ 하면 본 Phase 가 추가, 아니면 본 Phase 에서 신설.)

### 4.8 i18n namespace 트리 (`src/i18n/types.ts` 변경 없음)

phase-11 가 이미 `panels` namespace 등재. 본 Phase 는 _키 트리 확장_ 만 — 타입 모듈 변경 없음.
...
```ts
addFromFormula: (input: string, opts?: { signal?: AbortSignal }) =>
  Promise<Result<MoleculeId, IngestError>>;
previewFromText: (args: { kind: TextInputMode; raw: string; signal?: AbortSignal }) =>
  Promise<Result<Molecule, IngestError>>;
```
      ...
      <DialogContent className="max-w-2xl">
        ...
        <footer className="flex items-center justify-between gap-3 border-t border-border pt-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {replaceOn
                ? t('textInput.action.replaceActive')
                : t('textInput.action.addToWorkspace')}
            </Button>
          </div>
        </footer>
      </DialogContent>
...
export function PreviewPane({ preview, duplicate }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3" aria-live="polite">
      ...
      {duplicate.exists && <DuplicateWarning existingId={duplicate.existingId} />}
    </div>
  );
}
```
          ...
          <Button variant="secondary" size="sm" onClick={onAction}>
            {mapped.action === 'open-compound-browser' && (
              <>
                ...
                {t('panels.textInput.formulaUnsupported.openCompoundBrowser', { ns: 'panels' })}
              </>
            )}
            {mapped.action === 'reload-page' && (
              <>
                <RefreshCw size={14} className="mr-1" />
                {t('panels.textInput.rdkitInitFailed.reloadPage', { ns: 'panels' })}
              </>
            )}
          </Button>
        </div>
...
- `addFromMolecule(m)` 은 phase-11 §6.12 가 _내부에서_ `withRegeneratedIds(m, newId)` 자동 호출 → 호출자 (본 Phase) 추가 작업 없음.
...
- 단일 entry undoable. **kind 확정** (phase-07 `UndoableActionKind` 의 기존 멤버 사용 — 새 kind 추가 안 함): replace=off (새 slot) → **`'molecule.create'`**, replace=on (기존 slot 교체) → **`'molecule.replace'`**. (`'molecule.add-from-product'` 는 phase-07 union 에 없는 값이므로 사용하지 않는다 — 종전 "kind 재사용/분리 후속 결정" 미결 항목은 본 확정으로 닫힘.) withRegeneratedIds + setActive(newId) 는 그대로.
- duplicate 검출은 본 Phase 의 모달이 _제출 전_ `useDuplicateCheck` 로 검사 (D-DUPLICATE-CHECK 옵션 b). 매칭 시 Submit 버튼 disabled + DuplicateWarning 표시 + "기존 분자 활성화" 버튼.
...
import { useTranslation } from 'react-i18next';
import { Spinner } from '@/components';
// 좁은 read-only 모듈만 import (배럴 금지 — §7.3 ESLint 가드).
import { onRdkitStatusChange, getRdkitStatus } from '@/engine/rdkit/status';
import type { RdkitStatus } from '@/engine/rdkit/types'; // phase-03 §4.1

function getSnapshot(): RdkitStatus {
  // phase-03 §5.1 (`src/engine/rdkit/index.ts`) 의 RdkitService — `getRdkitStatus()` 는
  // `getRdkit().getStatus()` 의 thin 래퍼 (동기 read-only 상태 접근).
  ...
  switch (e.kind) {
    ...
    case 'pubchem':
      ...
      return {
        ...
        params: { message: e.detail.message ?? 'pubchem' },
      };
    case 'duplicate':
      return {
...
src/chemistry/compounds/regenerateIds.ts        (신규 — phase-11 인라인 분리)
...
- `seedHelpers.ts` 의 `getElementInitialFormula` — `panels/PeriodicTable/ElementInfoSheet.tsx` 가 _동일 panels/_ 레이어 간 참조\* 로 직접 import.

`@/chemistry/compounds/regenerateIds` 노출:
...
**예외 1개**: `panels/TextInput/RdkitStatusInline.tsx` 가 **`@/engine/rdkit/status`** (전용 thin 모듈) 의 `onRdkitStatusChange` + `getRdkitStatus()` 직접 import (배럴 `@/engine/rdkit` 아님 — §7.3). 정당화:

- `onRdkitStatusChange` = read-only 구독 함수 (이벤트 emitter 등록).
- `getRdkitStatus()` = `getRdkit().getStatus()` 의 thin 래퍼, 동기 read-only 상태 접근 (phase-03 §5.1).
- 동작 함수 (parseSmiles, toMoleculeWith3D 등) 는 _직접 import 안 함_.
- store 통과 패턴 (uiStore.rdkitStatus + onRdkitStatusChange subscriber 가 store action dispatch) 은 _과잉 설계_ — 모달 닫혀 있을 때 store 슬롯 무의미하고, RdkitStatus 는 phase-03 이미 _전역 싱글톤_.
...
`.eslintrc` (phase-11 §7.2 의 가드에 _부분 보완_):
  ...
  rules: {
    'import/no-restricted-paths': ['error', {
      zones: [
        ...
        // phase-12 추가:
        {
          ...
          from: './src/engine',
          // 본 패널은 **좁은 read-only 상태 모듈** 만 허용. 배럴 전체(`engine/rdkit/index.ts`)를
          // 화이트리스트하면 거기서 re-export 되는 동작 함수(parseSmiles/toMoleculeWith3D 등)도
          // 무심코 import 가능 → P1 우회. 따라서 **전용 thin 모듈만** except.
          except: ['./src/engine/rdkit/status.ts'],
        },
    ...
    'no-restricted-imports': ['error', {
      patterns: [
        // ... phase-11 의 기존 패턴 ...
        {
          // 배럴 `@/engine/rdkit` 에서 동작 함수가 새어 들어오는 것을 named-specifier 수준에서 차단.
          group: ['@/engine/rdkit', '@/engine/rdkit/index'],
          importNames: [
            'parseSmiles', 'parseInchi', 'parseFormula', 'parseSdfBlock',
            'toMoleculeWith3D', 'smilesTo3DMolecule', 'formulaToParsedMol',
            'createMainThreadRdkitBackend',
          ],
          message: 'panels 는 RDKit 동작 함수를 직접 import 할 수 없다. store 액션(addFromSmiles 등) 경유. 상태 구독은 @/engine/rdkit/status 의 onRdkitStatusChange/getRdkitStatus 만.',
        },
      ],
    }],
  },
...
**전용 모듈 신설** `src/engine/rdkit/status.ts` — `onRdkitStatusChange` 와 `getRdkitStatus(): RdkitStatus` (= `getRdkit().getStatus()` 의 thin 래퍼) **두 read-only export 만** 노출. 동작 함수는 이 모듈에 없으므로 file-level except 가 새지 않는다 (배럴 except 보다 엄격). `RdkitStatusInline` 은 `@/engine/rdkit/status` 만 import. phase-08 의 viewport/\* → chemistry/elements 직접 import (read-only / 정적 데이터 한정) 패턴과 동일 강도이되 **표면이 좁아 P1 우회 불가**.
...
| **R12** | ESLint `engine/rdkit` 직접 import 화이트리스트 _과다 완화_                                                        | panels/TextInput 의 다른 파일이 동작 함수 직접 import 가능            | §7.3 의 except 가 `engine/rdkit/index.ts` _file-level_ — `engine/rdkit/parse.ts` 등 sub-file 차단 유지. RdkitStatusInline 만 본 import 사용 — 다른 컴포넌트 검증은 PR 리뷰.                                                |
...
- [x] `<TextInputDialog>` Dialog mount + Esc + focus 복원 (C1, E1) — _impl ✓ / C1·E1 phase-15_
- [x] ModeTabs 전환 + raw reset (C2, E1/E5) — _impl ✓ / C2·E1·E5 phase-15_
- [x] InputArea (single-line / textarea) + autoFocus (A2) — _impl ✓ / A2 phase-15_
- [x] PreviewPane AsyncState 4 분기 (C3, C4, C10) — _impl ✓ / C3·C4·C10 phase-15_
- [x] DuplicateWarning + "기존 분자 활성화" (C6, E4) — _impl ✓ / C6·E4 phase-15_
- [x] ErrorMessage IngestError 분기 + 액션 버튼 (C5, C12) — _impl ✓ / C5·C12 phase-15_
- [x] ReplaceSwitch + active null 시 disabled (C7, R3) — _impl ✓ / C7 phase-15_
- [x] RdkitStatusInline (C10) — _impl ✓ / C10 phase-15_

### 10.2 PanelRegistry 등록

- [x] `registerAllPanels()` 가 6 개 패널 등록 (5 기존 + text-input) — `src/panels/registerAll.ts`
- [x] PanelKey 유니온에 `'text-input'` 추가 (U5) — `src/stores/uiStore.types.ts`
- [x] `<ModalHost>` 가 isTextInputOpen 토글 시 lazy 마운트

### 10.3 Store retrofit

- [x] `uiStore.panels.isTextInputOpen` + `textInputInitial` + 액션 2 개 (U9) — `src/stores/uiStore.ts`
- [x] `moleculeStore.addFromFormula(input, opts?)` (U3) — `src/stores/moleculeStore.ts`
- [x] `moleculeStore.previewFromText({kind, raw, signal?})` — state commit 없음 (U6) — `src/stores/moleculeStore.ts`
- [x] `mapIngestErrorToKey(e): IngestErrorMapping` (U1) — `src/stores/moleculeStore.selectors.ts`
- [x] `MoleculeCardSource` 유니온 확장 (`'text-input'` 추가) — `src/panels/_shared/MoleculeCard.tsx`
- [x] `src/chemistry/compounds/regenerateIds.ts` 신규 + barrel export (U7) — _phase-11 에서 이미 분리됨_
- [x] phase-11 §6.12 인라인 `withRegeneratedIds` → `regenerateIds` import 변경 (소급) — _phase-11 commit eddc69d_

### 10.4 Trigger 교체

- [x] Toolbar `<EditGroup>` 의 onSmiles → `toggleTextInput(true)` (C9) — `src/panels/Toolbar/EditGroup.tsx`
- [x] PeriodicTable `<ElementInfoSheet>` 의 onStartMolecule → `setTextInputInitial + toggleTextInput + togglePeriodicTable(false)` (C8, E2) — `src/panels/PeriodicTable/index.tsx`

### 10.5 i18n

- [x] `panels.textInput.*` 키 트리 한·영 양쪽 채움 (§5.5)
- [x] `common.ingest.error.*` 분기 한·영 양쪽 채움 (§5.4)
- [x] **dead placeholder 키 2 개 제거** — `panels.toolbar.smilesInputPlaceholder`, `panels.periodicTable.element.startMoleculePlaceholder`
- [x] `panels.toolbar.smilesInput` 라벨 보존 (버튼 라벨)

### 10.6 Placeholder trigger 도달 불가 검증

- [x] C9 컴포넌트 테스트가 `notify` mock spy 로 placeholder messageKey 호출 0 회 검증 — `tests/component/panels/ToolbarBar.test.tsx:144` (SMILES → isTextInputOpen=true + notifications=0)
- [ ] C8 동일 (PeriodicTable trigger) — _phase-15 test_
- [x] grep `'smilesInputPlaceholder'` / `'startMoleculePlaceholder'` codebase 잔존 0 검증

### 10.7 phase-11 인계 닫음

- [x] (a) Toolbar SMILES Input trigger placeholder → 모달 trigger 교체
- [x] (b) `<MoleculeCard>` 재사용 — preview 표시 (`source="text-input"`)
- [x] (c) `<Dialog>` + `<Input>` + `<Spinner>` + `<Tabs>` primitives 조립
- [x] (d) `<PanelErrorBoundary slotName="modal">` 가 모달 포착 (phase-10 ModalHost 정합)
- [x] (e) phase-09 isTextInputTarget 가드 자동 통과 검증 (C11, E8) — _impl ✓ (native input/textarea) / C11·E8 phase-15_
- [x] (f) PeriodicTable "이 원소로 분자 시작" trigger placeholder 교체

### 10.8 phase-10 인계 닫음

- [x] (a) `<Dialog>` Root/Content/Title/Close 사용 — ModalHost 자동 wrap
- [x] (b) `<Input>` + `<Spinner>` + `<Tabs>` + `<Switch>` + `<Button>` + `<Tooltip>` + `<Label>` + `<VisuallyHidden>` 사용
- [x] (c) Esc 닫기 + focus 복원 Radix 표준 (A1, C1) — _impl ✓ / A1·C1 phase-15_
- [x] (d) `<PanelErrorBoundary slotName="modal">` 사용 — ModalHost 정합
- [x] (e) isTextInputTarget 자연 통과 (C11) — _impl ✓ (native input/textarea) / C11 phase-15_

### 10.9 phase-09 인계 정합

- [x] (a) D10 isTextInputTarget 자동 통과 검증 (C11) — _impl ✓ / C11 phase-15_
- [x] (b) KEY_MAP 변경 없음 — 기존 키 충돌 없음 (D12)

### 10.10 phase-07 인계 닫음

- [x] addFromSmiles / addFromInchi / addFromFormula 모든 진입점 사용
- [x] IngestError 6 분기 표 i18n 매핑 완성 (mapIngestErrorToKey)
- [x] previewFromText action retrofit
- [x] AsyncState 패턴 일관 (preview 도 panel-local AsyncState)

### 10.11 라이브러리 / ESLint 가드

- [x] 신규 라이브러리 0 — lucide-react (phase-11 확정) 만 사용
- [x] ESLint `import/no-restricted-paths` 화이트리스트 1 항목 추가 (panels → engine, except rdkit/status.ts) + `no-restricted-imports` named-specifier 차단 (parseSmiles/toMoleculeWith3D 등 8 개)
- [x] phase-11 의 다른 zone 변경 없음
- [ ] 번들 영향 — 초기 chunk +0 KB (panels/TextInput = lazy chunk) — _phase-14 실측_

### 10.12 인계 가능성

- [x] phase-13 가 본 Phase 의 Dialog + Input + Spinner 패턴 재사용으로 JSON Import 모달 구현 가능
- [x] phase-13 가 `addFromMolecule` 호출 경로 추가 (JSON import) — 본 Phase retrofit 변경 없이
- [x] phase-13 가 옵션 (a) duplicate 검출의 store-internal 통합 retrofit 결정 (§11.9)
- [x] phase-14 가 Cmd+L 등 단축키 추가 + 입력 히스토리 등 _추가_ 만으로 확장 가능
- [x] phase-15 가 a11y axe + i18n 폴리싱 _추가_ 만

### 10.13 테스트 커버리지 (v0.2 진행 정합)

- [x] **검증 게이트 통과**: `pnpm lint` / `pnpm typecheck` / `pnpm test --run` (444/444 pass — phase-11 신규 4 파일 무회귀).
- [x] **C9 (Toolbar SMILES trigger 교체) 검증**: `tests/component/panels/ToolbarBar.test.tsx` 가 placeholder messageKey 호출 0 회 + `isTextInputOpen=true` 검증.
- [ ] U1 (mapIngestErrorToKey) / U2 (seedHelpers) / U3 (addFromFormula) / U4 (usePreviewParse) / U5 (PanelKey) / U6 (previewFromText) / U7 (withRegeneratedIds) / U8 (useDuplicateCheck) / U9 (toggleTextInput) — phase-15 polish.
- [ ] C1–C8, C10–C12 컴포넌트 테스트 — phase-15 polish.
...
2. **다중 SMILES 동시 입력** — "CCO.CCN" 같은 dot-notation 분리 SMILES 를 _N 개 분자로 분리_ 추가. 본 Phase v1 = 단일 분자 (RDKit 가 disconnected fragments 로 처리). 사용자 요구 강도에 따라 Phase 14+.
...
- **phase-11 (UI Panels)**:
  ...
  - `§6.12` 의 인라인 `withRegeneratedIds` 정의 제거 → `import { withRegeneratedIds } from '@/chemistry/compounds/regenerateIds'` 로 변경.
- **phase-09 (Interactions)**: 변경 없음 (D10 자동 정합).
- **phase-08 (Rendering)**: 변경 없음. (`createAtomId` / `createBondId` 는 phase-08 이 아니라 Phase 01 `@/chemistry/compounds/ids` 소속 — CD1.)
- **phase-03 (Chemistry Engine)**: Phase 03 의 _기존_ 코드/타입은 변경 없음. **본 Phase 가 신설하는 thin 모듈 1개**: `src/engine/rdkit/status.ts` — `onRdkitStatusChange` 재노출 + `getRdkitStatus()` (= `getRdkit().getStatus()` wrapper, phase-03 §5.1). 동작 함수 0개 (§7.3 ESLint 가드 대상). engine/rdkit 디렉터리에 read-only 표면만 추가하는 **phase-12-소유 모듈** (arch §4 레이아웃은 예시적 — 동일 레이어 additive 허용).
...
**없음**. 본 Phase 가 architecture 의 다음 약속을 _경로 닫음_ 으로 이행:
...
_문서 버전: 0.2 (구현 정합)_
_작성일: 2026-04-30 (v0.1 초안) / 2026-05-18 (v0.2 — 정합 검증)_

> **v0.2 변경 요약:** 본 Phase 는 undo dispatcher / rrp / fade 식별자·경로 drift
> **없음** (검증 완료): `useUndoableDispatcher` 는 store action 경유 간접 참조로
> 정확 기술(§3.1 인계표 10 행), text-input 은 native `<input>` 라 phase-09 D10
> 가드 자동 통과. `withRegeneratedIds`(CD1)는 본 Phase 신설·소유 — 위치
> `src/chemistry/compounds/regenerateIds.ts` (phase-13 §12.1 와 일치).
> phase-09/10 v0.2 코드와 무모순. 내용 변경 없음 (버전 표기만 갱신).
