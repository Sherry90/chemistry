...
export interface Element {
  ...
  readonly meltingPointK: number | null; // 실험값만 (예측값 아님)
  readonly boilingPointK: number | null;
  // D2: 합성 원소 등 실험값 부재 시 예측값 별도 필드. v1 은 optional·기본 미수록
  //     (값 없으면 키 자체 생략 — exactOptionalPropertyTypes 정합). §11 Q4 가 수록 여부 확정.
  readonly predictedMeltingPointK?: number;
  readonly predictedBoilingPointK?: number;
...
> **`sourceVersions` 의 용도**: 외부 표준 개정 (예: IUPAC 원자량 2021→2024, NIST ASD 갱신) 을 _명시적으로_ 추적. 수집 스크립트는 자신이 사용한 소스 버전을 이 필드에 기록하며, CI 또는 정기 점검 워크플로 (Phase 15) 가 최신 표준 버전과 비교해 _재수집 권장_ 알림을 띄울 수 있다. 실제 재수집 시 골든 데이터 변경은 PR 로 명시적 검토.
...
```ts
/**
 * `n` 은 brand `ElementNumber` (1..118 — Phase 01 `isValidElementNumber` 통과한 값만 brand 획득).
 * 유효 brand 에 대해 항상 비-null `Element` 반환. **불변식 위반(위조 brand 등)은 프로그래밍 오류**로
 * 간주하여 `throw`(arch §3.7 의 "예외 throw 는 불변식 위반 한정" 정합) — 도메인 실패 아님.
 * 검증되지 않은 정수는 `getElementUnsafe` 사용 (그쪽이 `| undefined`).
 */
...
> **D6 의 La/Ac 동시 소속**: `block` 과 `category` 는 _독립 축_ 이다. La(57) 과 Ac(89) 는 `block: 'd'` 이면서 `category: 'lanthanide'` / `'actinide'` 이므로 다음 호출이 모두 La 를 포함한다:
>
...
export function elementNameOf(n: ElementNumber, locale: Locale): string; // Locale = Phase 01 @/types/settings ('ko'|'en')
...
8. **D6 규약 검증**: `getElementsByBlock('f').length === 28` 이며, La(57)/Ac(89) 의 `block === 'd'` 및 `category ∈ {'lanthanide','actinide'}` 임이 유닛 테스트로 확인됨. **추가 교차 검증**: `getElementsByBlock('d').length === 40` (Sc, Y, La, Ac 포함) 이고, `getElementsByCategory('lanthanide').length === 15` (La 포함, Ce–Lu 14 + La 1) — La/Ac 가 두 필터 모두에 _동시 존재_ 함이 의도적임을 테스트가 명시.
