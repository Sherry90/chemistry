...
| **09 (Interactions)**     | `UndoableDispatcher` 인터페이스 + `UndoableActionKind` 9개 enum, `moveAtom` / `setBondOrder` / `addBond` 등 동기 편집 액션 시그니처. Phase 09 는 swappable `dispatcher` 싱글톤(`_shared/undoable.ts` export 변수, §659–664 패턴)의 내부 구현을 `createUndoStack()`(stores 레이어 `_shared/undo/`)으로 `setUndoDispatcher()` 주입 — 식별자·인터페이스·9개 액션 본문 불변 (phase-09 §5.1/§12, v0.2 정합). ambient group API 도 본 파일에 추가(drag 전용, 액션 meta 불변) |
