...
1. **6 종 패널 본문 구현** — `panels/PeriodicTable`, `panels/CompoundBrowser`, `panels/Conditions`, `panels/MoleculeInfo`, `panels/ReactionResult`, `panels/Toolbar`. Phase 10 의 `<AppLayout>` 슬롯 + `PanelRegistry` + 13 종 primitives 위에 _실제 UI_ 와 store 연결을 얹는다.
2. **PanelRegistry 등록** (phase-10 D14 약속의 닫음) — `registerAllPanels()` 부팅 진입점이 5 개 패널 (`registerPanel(def)`) 을 등록. Toolbar 는 _직접 주입_ 이라 등록 제외.
...
5. **Q1 ReactionResult product 적재 정책 닫음** (phase-09 §11 #11 / D16 인계의 닫음) — 명시적 "작업공간에 추가" 버튼. `moleculeStore.addFromMolecule(m)` / `addFromMolecules(ms)` retrofit. 적재 시점에 phase-08 `useFadeOnMount` 가 자연스럽게 fade-in 발화 (별도 reaction-trigger crossfade 드라이버 불요 — `architecture.md` §1.3 ⑥ "결합 변화 애니메이션" 약속의 _경로_ 확정).
...
12. **i18n `panels.*` 네임스페이스 채움** — 한·영 양쪽. `common.json#reaction.*` (phase-06 §4.8) 와 `shortcuts.*` (phase-09 §4.4) 는 _재사용_ 만 — 새 키 정의 안 함. 한·영 문구 최종 폴리싱은 Phase 15.

본 Phase 는 `architecture.md` §12 의 열린 질문 중 **#1 (반응 예측 한계 UI 문구)** 의 ReactionResult 표면을 _조립_ 으로 닫는다 (phase-06 §4.8 가 정의한 `experimentalBadge` / `experimentalDisclaimer` 키 + Tooltip + 모달 디스클레이머 표시). **#2 (큐레이션 정책)** / **#3 (ΔH)** / **#5 (WebGL2)** / **#6 (이온)** 은 본 Phase 영역 외.
...
- **반응물 선택 + 실행** (D-REACTION-RUN): 패널 하단에 작업공간 분자 목록(체크 토글 → `setReactants(ids)` / `addReactant` / `removeReactant`, `selectReactantIds` 구독)과 **"반응 실행" 버튼**을 둔다. 버튼은 반응물 ≥ 1 이고 `selectIsRunning === false` 일 때만 활성. onClick → `reactionStore.run()`; **성공 시 `uiStore.toggleReactionResult(true)` 로 ReactionResult 모달 자동 열기** (D-REACTION-RESULT-AUTO-OPEN), 실패 시 `notify({ level:'error', messageKey: mapReactionErrorToKey(error) })`. 이 패널이 반응 기능의 **단일 진입점**이다 (Toolbar 에 별도 슬롯 추가하지 않음).
...
- **`<ReactionResultPanel/>`** — modal Dialog (PanelKey `'reaction-result'`, mode='modal'). `<ConditionsPanel>` 의 "반응 실행" 성공 시 `toggleReactionResult(true)` 로 자동 열림 (D-REACTION-RESULT-AUTO-OPEN 확정).
...
- **rule 탭**:
  - `kind === 'rule-based'` → **`appliedRuleId` 만** 표시 (`reactionResult.rule.appliedRule`, `{{id}}`). `ReactionResult` 계약에 `source`/규칙 메타가 없으므로 v1 은 출처/version/license 비표시 (CD5). 출처 메타 노출은 §11 #13 후속 (Phase 06 가 ReactionResult 에 메타 추가 또는 별도 manifest 조회 결정 시).
...
- **action 매핑**:
  - Undo/Redo **표시** (활성/비활성 토글): `useUndoStack(): { canUndo, canRedo }` (phase-09 §5.3 권장 (1) 의 selector 훅 — React 반응성 보장). **발화** (실제 undo/redo 실행): `useMoleculeStore(s => s.actions.undo / redo)` — phase-07 §5.1 의 store action 이 정통 경로. phase-07 §6.4 / §659–664 의 동결 약속에 따라 본 wrapper 는 내부적으로 dispatcher 를 호출하며, phase-09 가 _export 시점에_ 본 dispatcher 구현으로 swap (호출자 코드 무변경). 별도 `useUndoableDispatcher()` 컨텍스트 (phase-10 D17) 는 _비-store API_ (예: phase-13 importJson 의 `dispatcher.clear()`) 호출용 — Undo/Redo 자체는 store action 경로가 phase-07/09 동결. `viewportApi.canUndo()` / `canRedo()` 는 imperative 보조 (호출 시점 값 — React 자동 리렌더 안 됨).
...
- `registerAllPanels(): void` — App.tsx 가 `<AppLayout/>` 마운트 _전_ 1 회 호출.
...
- 이 단축키의 발화 경로는 D-SHORTCUT-OPEN-STATE 확정: phase-10 `installAppShortcuts` 단일 리스너가 `onAction('showShortcutSheet')` 콜백으로 통지 → Toolbar 로컬 Popover open 토글 (별도 Toolbar keydown listener 금지, KEY_MAP 단일 소스).
...
| product 적재 시점의 _명시적 reaction-trigger crossfade 드라이버_ (mount fade-in 으로 충족) | 14 (D16 후속)                           |
...
| ShortcutSheet open 상태 store 위치            | —                                                                                                                        | **확정**: D-SHORTCUT-OPEN-STATE (Toolbar 로컬 useState, `installAppShortcuts.onAction` 단일 디스패치) |
...
| **06 (Reaction Engine)**                                                                                                                                                       | _직접 import 안 함_. `ReactionResult` (Phase 01 단일 정의 = `{ products, kind, appliedRuleId, thermo, confidence: number, notes: string \| null (i18n 키) }`), `ReactionEngineError`, `ThermoFlag`, `ReactionPredictionKind` 타입은 phase-07 selector / action 결과 타입으로 흘러옴. `notes` 가 i18n 키이므로 표시 시 `t(notes)`. **i18n 키 재사용**: `common.json#reaction.experimentalBadge` / `experimentalDisclaimer` (phase-06 §4.8) — 본 Phase 는 t() 호출만, 새 키 정의 안 함. |
| **05 (Runtime Data)**                                                                                                                                                          | _직접 import 안 함_. `PubChemError` 의 retryable 분류는 phase-07 의 `compoundSearch.results` AsyncState 의 error 분기로 노출.                                                                                                                                                                                                                                                                                                                                                         |
| **07 (Stores)**                                                                                                                                                                | 본 Phase 의 _주된 의존_. **모든 패널이 stores 만 통과해 도메인 데이터에 접근** (P1 정합).                                                                                                                                                                                                                                                                                                                                                                                             |
...
- **moleculeStore action** (시그니처 핀 — phase-07 §5.1 확정 요구): `addFromSmiles(input, opts?): Promise<Result<MoleculeId, IngestError>>`, `addFromInchi(...)` 동형, **`addFromCompound(cid: number, opts?): Promise<Result<MoleculeId, IngestError>>`** (`result.ok` / `result.error.kind` 분기 — `IngestError.kind ∈ {parse,embed,pubchem,duplicate,rdkit-not-ready,internal}`), `setActive`, `removeMolecule`, `replace`, `moveAtom`, `setBondOrder`, `addBond`, `removeBond`, `addAtom`, `removeAtom`, `clear`, `undo`, `redo`, `addFromMolecule(m): MoleculeId` (retrofit), `addFromMolecules(ms): readonly MoleculeId[]` (retrofit). **phase-07 retrofit 요구**: `lastUndoSeq: number` (undo 마다 +1, redo/일반 변경 불변 — D-UNDO-TOAST §6.13 단일 관찰자용).
...
- **uiStore selector**: `selectActivePanel`, `selectAtomLabelsOn`, `selectBackgroundOverride`, `selectNotifications`, `selectIsGloballyLoading`, `selectCompoundSearch`, **`selectSelection`** (→ `{ atomIds: readonly string[]; bondIds: readonly string[] }`), **`selectHasSelection`** (→ `boolean`). `selection` slice 형태는 phase-07 uiStore 가 단일 정의 — 본 Phase 는 `.getState()` 직접 접근 대신 이 selector 사용 (EditGroup/§6.13 단일 관찰자). `selectCompoundSearch` → `CompoundSearchSlice = { query: string; mode: 'name'|'cid'|'formula'; results: AsyncState<readonly Compound[], PubChemError>; selectedCid: number | null }` (phase-07 확정 요구; `results.kind==='success'` 일 때 `.value: readonly Compound[]`).
...
- **공용 타입**: `Notification`, `PanelKey`, `CompoundSearchSlice`, `AsyncState<T,E>`, `MoleculeId`. |
  | **09 (Interactions)** | `@/viewport` 배럴: `KEY_MAP: ReadonlyArray<KeyBinding>`, `KeyBinding { id, scope, matches, action, i18nLabelKey }`, `KeyActionId` 유니온, `describeKey(binding): string`, `BondCreateFlowApi` (참조만 — Toolbar 는 `viewportApi.createBondFromSelection()` 호출). **(v0.2)** `useUndoStack(): { readonly canUndo: boolean; readonly canRedo: boolean }` 는 `@/stores` 배럴 (phase-09 v0.2 가 undo 코어 stores 이전). **retrofit 대상**: `KEY_MAP` 에 `?` (showShortcutSheet) 추가 + `KeyActionId` 유니온 확장. |
  ...
  | **08 (Rendering)** | _직접 import 안 함_. `ViewportApi` 타입만 (`@/viewport` barrel — phase-08 §5.2 + phase-09 §5.3 retrofit 포함): `frameActive`, `frameAll`, `resetCamera`, `captureBlob({ format, dpr?, transparentBackground? }): Promise<Blob>`, `getRenderer()`, `canUndo()`, `canRedo()`, `createBondFromSelection(): boolean`. |

본 Phase 는 `engine` / `services` / `viewport/*` _내부_ 를 import 하지 않는다 (P1 정합). `chemistry/elements` 와 `data/compounds` 는 _정적 데이터_ 한정 예외.

### 3.2 외부 라이브러리 (architecture §66 신규 도입 절차)

본 Phase 는 phase-10 가 §3.2 에 _잠정_ 등재한 `lucide-react` 를 **확정** 표명한다. **신규 라이브러리 0**.
...
(개수 22 — 일부 중복 + 본 표는 _최대_ 추정. 실제 import 는 §6 에서 컴포넌트 단위로 명시.)
...
| **P2**  | Toolbar 는 `registerPanel` 으로 _등록 안 함_. `<AppLayout toolbar={<ToolbarBar/>}>` prop 으로 직접 주입. PanelKey `'toolbar'` 값은 본 Phase 에서 **예약 미사용** — phase-12+ 가 다른 용도 (예: SMILES 입력 패널의 active 표시) 로 쓸 수 있음.                                                                                                                                                 | phase-10 §6.1 / phase-07 §4.4       |
...
| **P5**  | 도메인 실패는 Result 패턴으로 store 의 AsyncState 분기에 흘러옴. 본 Phase 의 패널은 _AsyncState 4 분기 (idle/loading/success/error) 를 UI 분기_ 로 표현. `<PanelErrorBoundary>` 는 _런타임 예외_ 만 포착 (예: 잘못된 ref 접근, undefined property).                                                                                                                                           | architecture §3.7 / phase-10 P5     |
...
| **P11** | `panels.active` (단일 docked) ↔ `is*Open` (개별 modal) 동시 사용 정책. 본 Phase 는 _selector 만 사용_ — 두 슬롯의 의미 분기는 phase-07 가 보장 (phase-10 P10 정합). 동시 표시는 의도된 디자인 (docked = 지속, modal = 일시).                                                                                                                                                                  | phase-07 §4.4 / phase-10 P10        |
| **P12** | 토스트 큐 길이 상한 50 — phase-07 R7 가 보장 (50 도달 시 가장 오래된 자동 dismiss). 본 Phase 의 ToastContainer 는 _모든_ 항목 렌더하지만 큐 길이가 보장되어 안전.                                                                                                                                                                                                                             | phase-07 R7 / phase-10 P9           |
...
| **D-UNDO-TOAST**                   | undo 후 selection 손실 안내 토스트 (phase-09 §11 #8) | **표시함 — 단일 관찰자 경로 (Toolbar 버튼·`Cmd+Z` 동일 동작)**. Toolbar 버튼 onClick 이 토스트를 직접 발화하지 **않는다** (버튼만 발화 시 `Cmd+Z` 와 불일치). 대신 `<ToolbarBar>` 가 1회 마운트하는 `useUndoSelectionToast()` 훅이 **moleculeStore 의 `canUndo`/히스토리 길이 변화를 구독**하여, undo 가 발생했고 _그 직전_ `selection` 이 비어있지 않았으면 `notify({ level:'info', messageKey:'panels.toolbar.undoSelectionRestored.notice', dismissAfterMs:3000 })` 발화. throttle: 200 ms (phase-09 D3 그룹 윈도우와 동일, timestamp ref). 트리거 출처(버튼/단축키) 무관 — 단일 관찰자라 일관. (대안으로 phase-09 `installAppShortcuts` 에 `onUndoFired?` 콜백을 두는 방법도 동일 효과 — 본 Phase 는 store 구독 관찰자 채택, retrofit 불요.) | §6.6                                                  |
...
| **D-PANELS-INDEX**                 | `panels/index.ts` 부팅 진입점                        | `registerAllPanels(): void` 함수. `App.tsx` 가 `<AppLayout/>` 마운트 _전_ 1 회 호출. phase-10 §4.1 의 PanelRegistry 가 _부팅 1 회 채워진 뒤 변하지 않는다_ 는 가정 정합. 동일 key 중복 등록 시 phase-10 의 `registerPanel` 이 logger.warn (phase-10 §5.2).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | §6.10                                                 |
| **D-SHORTCUT-OPEN-STATE**          | ShortcutSheet open 상태 위치 + `?` 디스패치 경로     | **Popover open 상태**는 Toolbar 컴포넌트 내부 `useState` (uiStore 불필요 — SSOT 가 Popover 자체). **`?` 디스패치는 단일 경로**: 별도 Toolbar keydown listener 를 만들지 않고, phase-10 의 **`installAppShortcuts({ dispatcher, onAction })`** 단일 글로벌 리스너(= phase-09 text-input gate + undo 디스패치를 이미 감싸는 그 리스너)가 KEY_MAP 의 비-undo 바인딩을 `onAction(actionId)` 콜백으로 통지한다. App.tsx 가 `installAppShortcuts` 설치 시 `onAction` 을 주입하고, Toolbar 는 `onAction('showShortcutSheet')` → 로컬 Popover open 토글하는 핸들러를 등록(경량 pub/sub 또는 ref). KEY_MAP 이 **유일한 단축키 소스**로 유지되고 text-input gate 중복 없음.                                                                                | §6.6 / §6.11                                          |
| **D-REACTION-RESULT-AUTO-OPEN**    | ReactionResult 모달 자동 열기 정책                   | **확정 (v1)**: 반응 진입점은 `<ConditionsPanel>` 하단의 "반응 실행" 버튼 (D-REACTION-RUN) 단일. 핸들러는 `await reactionStore.run()` → **성공 시 `uiStore.toggleReactionResult(true)` 로 모달 자동 열기**, 실패 시 토스트만 (모달 안 염). 이로써 ReactionResult 패널이 **항상 도달 가능**하다 (종전 "호출자가 알아서 toggle" 의 미해결 갭 제거). 사용자가 모달을 닫아도 `lastResult` 보존(D-REACTION-RESULT-CLEAR), Conditions 패널에서 재실행하면 다시 자동 열림. Phase 14+ 의 settingsStore.autoOpenReactionResult opt-out 토글은 후속 확장 (v1 은 항상 자동 열기).                                                                                                                                                                            | §6.4 ConditionsPanel / §11 #2                         |
...
| **D-LOG-RUNTIME-ERRORS**           | 패널 내부 런타임 예외 logging                        | `<PanelErrorBoundary panelKey/>` (phase-10 §6.5) 가 자동으로 `logger.error('panel-boundary.caught', ...)` 호출. 본 Phase 패널 컴포넌트는 _추가 logger 호출 안 함_ — Boundary 가 책임. 도메인 실패 (Result.error) 는 패널 UI 의 error 분기로 표시 (logger 호출 안 함 — 정상 흐름).                                                                                                                                                                                                                                                                                                                                                                                                                                                                | §6 전반                                               |
...
본 Phase 는 새 도메인 타입을 _최소_ 도입한다 (재사용 우선 정책 §3 적용 원칙 #3).
...
export interface MoleculeStoreActions {
   ...
   * - 새 MoleculeId 생성: Phase 01 `createMoleculeId()` (`@/chemistry/compounds/ids`, CD1).
   * - atoms / bonds 의 stable ID 도 새로 생성: Phase 01 `createAtomId()`/`createBondId()`
   *   (`@/chemistry/compounds/ids`) — 호출자 책임이 아니라 본 store action 이 자동 생성.
   *   ReactionResult.products 의 분자가 이미 ID 를 가질 수 있으므로 *재할당* 이 안전).
...
**디스패치 (단일 경로)**: 본 바인딩의 키 처리는 **phase-10 `installAppShortcuts` 단일 글로벌 리스너**가 담당한다. 이 리스너는 phase-09 의 undo/redo 디스패치 + text-input gate 를 이미 감싸며, 비-undo 바인딩(`showShortcutSheet` 등)은 주입된 `onAction(actionId: KeyActionId)` 콜백으로 통지한다. **Toolbar 는 독립 keydown listener 를 만들지 않는다** (게이트 로직 중복·KEY_MAP 분기 소스화 방지 — D-SHORTCUT-OPEN-STATE). `installAppShortcuts` 시그니처는 `installAppShortcuts({ dispatcher, onAction }): () => void` 로 phase-10 §Shortcuts 에 retrofit (본 Phase 인계 — §12 (e)).

### 4.9 i18n 새 네임스페이스 (`panels.json`)

상세 키 트리는 §5.4 + §6.9 에서 정의. 본 §4 는 _최상위_ 만:
...
// en/panels.json
{
  ...
  "reactionResult": {
    ...
    "rule": { "appliedRule": "Applied rule: {{id}}" },
    "retry": "Retry",
  },
  "toolbar": {
    "undo": "Undo",
        ...
        <>
          ...
          <ElementInfoSheet
            ...
            onStartMolecule={() => {
              notify({
                level: 'info',
                messageKey: 'panels.periodicTable.element.startMolecule.placeholder',
                dismissAfterMs: 3000,
              });
            }}
          />
...
export function PeriodicGrid({ layout, onSelect, selected }: Props) {
  ...
  // Roving tabindex: 정확히 1개의 셀만 tabindex=0 (focusable), 나머지 -1.
  // 초기 focusable 셀 = selected 또는 첫 원소 (H, Z=1).
  const [activeZ, setActiveZ] = useState<number>(selected?.number ?? 1);

  // 화살표/Home/End 로 셀 간 이동 (WAI-ARIA grid 패턴). layout 은 (row, column 1..18).
  const moveFocus = (fromEl: Element, key: string) => {
    const cur = layout.find((c) => c.element.number === fromEl.number)!;
    const target = (() => {
      switch (key) {
        case 'ArrowRight':
          return layout.find((c) => c.row === cur.row && c.column === cur.column + 1);
        case 'ArrowLeft':
          return layout.find((c) => c.row === cur.row && c.column === cur.column - 1);
        case 'ArrowDown':
          return layout.find((c) => c.column === cur.column && c.row === cur.row + 1);
        case 'ArrowUp':
          return layout.find((c) => c.column === cur.column && c.row === cur.row - 1);
        case 'Home':
          return (
            layout.find((c) => c.row === cur.row && c.column === 1) ??
            layout.filter((c) => c.row === cur.row).sort((a, b) => a.column - b.column)[0]
          );
        case 'End':
          return layout.filter((c) => c.row === cur.row).sort((a, b) => b.column - a.column)[0];
        default:
          return undefined;
      }
    })();
    if (target) {
      setActiveZ(target.element.number);
      // ref 맵으로 실제 DOM focus 이동 (cellRefs[target.element.number]?.focus()).
    }
  };
...
**a11y (WAI-ARIA grid 패턴 — v1 포함)**: `role="grid"` + `aria-label`, 각 셀 `role="gridcell"` + `aria-label` (한·영 이름 + 원자번호) + `aria-selected`. **roving tabindex**: 정확히 1개 셀만 `tabIndex=0`, 나머지 `-1` — Tab 으로 그리드 진입/이탈은 1-stop, 그리드 내부 이동은 **화살표 키**(상하좌우 = 같은 column/row 인접 셀, 빈 칸은 건너뛰지 않고 해당 위치에 셀 없으면 이동 없음), `Home`/`End` = 현재 row 의 처음/끝, `Enter`/`Space` = 선택. arch §3.5 의 "키보드 affordance 는 10/11 에서 설계" 충족 (Phase 15 는 axe 자동 검증만). 셀 DOM focus 이동은 `cellRefs` 맵으로 수행.
  ...
  return (
    <div className="flex h-full flex-col gap-4 p-6">
      ...
      <div className="flex-1 grid grid-cols-2 gap-4 overflow-hidden">
        <ResultList search={search} onSelect={selectResult} />
            ...
            <MoleculeCard
              ...
              actions={
                selectedCompound.cid != null
                  ? { onAddToWorkspace: () => onAdd(selectedCompound.cid as CompoundId) }
                  : undefined /* CID 없는 항목은 추가 비활성 (MoleculeCard 가 버튼 숨김) */
              }
            />
          ) : (
...
`selectBondMetrics` 의 시그니처는 `(m: Molecule | null) => BondMetricsResult | null` (standalone 순수 함수, _non-curried_) — id 가 아닌 Molecule 자체를 받는다. 본 컴포넌트가 `selectActiveMolecule` 로 받은 Molecule 을 그대로 전달. 캐시는 WeakMap<Molecule, ...> + useMemo (§4.7 참조).
...
**RuleTab**: rule-based 이면 `appliedRuleId` 만 표시 (`t('reactionResult.rule.appliedRule', { id })`). source/version/license 는 `ReactionResult` 계약에 없으므로 v1 비표시 (CD5, §11 #13 후속). heuristic-experimental 이면 `<ExperimentalDisclaimer/>` (phase-06 §4.8).
...
import { useTranslation } from 'react-i18next';
import type { ViewportApi } from '@/viewport';
// (v0.2) useUndoStack 은 @/stores 배럴 (phase-09 v0.2 가 undo 코어를 stores
// 레이어로 이전). KEY_MAP/describeKey 는 @/viewport 배럴 유지.
...
export function EditGroup({ apiRef }: Props) {
  ...
  // D-UNDO-TOAST: 버튼은 토스트를 직접 발화하지 않는다 (Cmd+Z 와 일관성).
  // 토스트는 <ToolbarBar> 가 1회 마운트하는 useUndoSelectionToast() 단일 관찰자가
  // moleculeStore undo 발생 + 직전 selection 비어있지 않음 + 200ms throttle 로 발화.
  const onUndo = () => undo();
  ...
  const onSmiles = () => {
    notify({
      level: 'info',
      messageKey: 'panels.toolbar.smilesInput.placeholder',
      dismissAfterMs: 3000,
    });
  };
  ...
  return (
    <>
      <Tooltip
        content={`${t('toolbar.undo')} (${describeKey(KEY_MAP.find((b) => b.id === 'undo')!)})`}
      >
      ...
      <Tooltip
        content={`${t('toolbar.redo')} (${describeKey(KEY_MAP.find((b) => b.id === 'redo')!)})`}
      >
      ...
      <Tooltip
        content={`${t('toolbar.createBond')} (${describeKey(KEY_MAP.find((b) => b.id === 'createBondFromSelection')!)})`}
      >
        ...
        <PopoverContent>
          <div className="flex flex-col gap-1 p-2 min-w-40">
            <button
              ...
              className={cn(
                'text-sm px-2 py-1 rounded text-left',
            ...
            <button
              type="button"
              disabled
              className="text-sm px-2 py-1 rounded text-left text-fg-muted cursor-not-allowed"
            >
...
export function ToastItem({ notification }: Props) {
  ...
  // messageKey 규약: **항상 i18next 완전수식 `ns:key`** (예: 'panels:toolbar.undoSelectionRestored.notice',
  // 'shortcuts:bondCreate.diffMolecule', 'common:reaction.experimentalBadge'). notify() 호출측은
  // 반드시 ns 접두사를 포함한다. ToastItem 은 default `t` 로 호출하면 `:` 가 ns 를 선택하므로
  // 단일 호출로 panels/shortcuts/common 어느 ns 든 해석된다 (i18next nsSeparator=':').
  return (
    <div
      role={notification.level === 'error' || notification.level === 'warn' ? 'alert' : 'status'}
      aria-live={
        notification.level === 'error' || notification.level === 'warn' ? 'assertive' : 'polite'
      }
      className={cn(
...
진입 애니메이션 (slide-in) 은 부모 (ToastContainer) 가 mount 시 CSS class 토글 — 본 컴포넌트는 _exit_ 만 자체 처리. 또는 framer-motion 도입 여부는 §11 (현재는 단순 CSS transition).
...
본 Phase 는 `panels.{ko,en}.json` 에 §5.4 의 키 트리 전체를 채운다. _문구 자체_ 는 한·영 자연스러운 표현으로 작성하되, **Phase 15 가 최종 폴리싱** (예: `chemistry.ko.json` 의 화학 용어 사전과 일관성, 색약 모드 라벨 강제 on 검증, 단축키 표기 convention).

**기존 네임스페이스 retrofit**:

- `shortcuts.json` (한·영) 에 `bondCreate.diffMolecule` + `showShortcutSheet` 키 추가.

**chemistry.json (phase-02 가 정의)** 는 _재사용만_ — 원소명 한·영 (`chemistry.elements.{symbol}.{ko,en}` 또는 phase-02 의 실제 구조) 을 PeriodicTable / MoleculeInfo 가 호출.

**common.json (phase-06 가 정의)** 의 `reaction.experimentalBadge` / `reaction.experimentalDisclaimer` / `reaction.error.*` 도 _재사용만_.
...
**shortcuts.json 에 라벨 키 채움 의무**: phase-09 §4.4 의 `shortcuts.{action}` 키 들. 본 Phase 는 _기존 키_ 를 재사용 + `showShortcutSheet` 1 개 추가.
...
**meta 형태**: `UndoableMeta { kind, labelKey?, group? }` (phase-07 §3.2 의 인터페이스). `group` 키가 같은 연속 dispatch 는 phase-09 의 200 ms merge window 안에서 단일 entry 로 압축. `addFromMolecules` 는 batchId 로 group 키를 발급해 _동일 호출 안의 N 개 분자_ 를 1 개 entry 로 묶는다.

**`withRegeneratedIds`** (`src/chemistry/compounds/regenerateIds.ts` — CD1·CD6: chemistry 계층 순수 모듈, Phase 12 가 신설·phase-11/13 공용 import) 는 Phase 01 `createAtomId` / `createBondId` / `createMoleculeId` (`@/chemistry/compounds/ids`, CD1) 를 활용해 molecule/atoms/bonds 의 stable ID 를 일괄 새로 발급. ReactionResult.products 의 분자 ID 가 reactionStore 안에서 _임시_ 일 수 있어 재할당이 안전.

**fade-in**: phase-08 의 `useFadeOnMount` 가 `molecules[newId]` 의 mount 를 감지하면 자동 발화 — 별도 reaction-trigger 드라이버 불요. `architecture.md` §1.3 ⑥ "결합 변화 애니메이션" 약속의 _경로_ 가 이 시점에 닫힌다.

### 6.13 D-UNDO-TOAST 의 정확한 동작 (단일 관찰자 — 트리거 출처 무관)

토스트는 **버튼 onClick 에 결합하지 않는다** (버튼만 발화하면 `Cmd+Z` 와 불일치 — Agent 검토 지적). 대신 `<ToolbarBar>` 가 1회 마운트하는 `useUndoSelectionToast()` 단일 관찰자가 처리:

```ts
// panels/Toolbar/_shared/useUndoSelectionToast.ts
export function useUndoSelectionToast(): void {
  const notify = useUiStore((s) => s.actions.notify);
  const lastToastTs = useRef(0);
  // undo 발생 감지: moleculeStore 의 히스토리/스냅샷 변화를 subscribeWithSelector 로 구독.
  useEffect(() => {
    let prevSelectionNonEmpty = hasSelection(useUiStore.getState().selection);
    const unsub = useMoleculeStore.subscribe(
      (s) => s.lastUndoSeq, // moleculeStore 가 undo 마다 증가시키는 단조 카운터
      (seq, prevSeq) => {
        if (seq <= prevSeq) return; // undo 만 (redo/일반 변경 아님)
...
const hasSelection = (sel: { atomIds: readonly string[]; bondIds: readonly string[] }) =>
  sel.atomIds.length > 0 || sel.bondIds.length > 0;
```

- **트리거 출처 무관**: Toolbar 버튼이든 phase-09 `Cmd+Z` 단축키든 둘 다 `moleculeStore.undo()` 단일 액션을 거치므로, 그 액션이 증가시키는 `lastUndoSeq` 카운터를 구독하면 **두 경로 모두 동일하게** 토스트가 발화된다 (retrofit/콜백 불요).
- **phase-08 `useSelectionStaleGuard`** 가 undo 후 stale selection 을 자동 정리 — 본 토스트는 그 사실을 사용자에게 안내.
- **phase-07 인계 요구**: moleculeStore 에 `lastUndoSeq: number` (undo 시 +1, redo/일반 변경 시 불변) 노출 — phase-07 §5.1 retrofit (본 Phase §12 등재). selector `selectHasSelection` 도 phase-07 uiStore 에서 제공 (아래 CONSUMES).
...
`.eslintrc` (phase-10 §7.2 의 가드에 _추가_):
...
| U5  | `addFromMolecules`  | 그룹 undoable (batchId), 모든 분자 동시 추가, activeId = 마지막. undo() 후 _모두_ 제거.                                       |
...
| ID  | 대상                  | 검증                                                                                                                                                                                                                                                    |
| --- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | PeriodicGrid          | role="grid" + aria-label, 셀 role="gridcell" + aria-label (한·영 이름 + 원자번호 + selected). **roving tabindex**: 정확히 1셀 tabindex=0. ArrowRight/Left/Up/Down 로 focus 이동, Home/End row 끝, Enter/Space 선택 — 키 시뮬레이션으로 focus 이동 검증. |
...
| ID      | 시나리오                                                                                                                 | 영향                                                          | 대응                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **R1**  | `panels/*` 의 chemistry/elements 직접 import 가 ESLint 가드 _과다 완화_                                                  | 다른 panels 가 chemistry/bonds derive 함수도 임의 import 가능 | §7.2 의 `import/no-restricted-paths` `except` 가 `chemistry/elements/**` _명시_ — sub-directory 만 허용. chemistry/bonds 차단.                                                                                                                                                                                                                                                                            |
...
| **R9**  | ExperimentalBadge 가 CVD 모드에서 색상 의존                                                                              | architecture §3.5 위반 가능                                   | D-EXPERIMENTAL-BADGE-PLACEMENT 가 _색상 + 라벨 + `_` 마커\* 강제. phase-15 가 CVD 모드에서 패턴/심볼 추가 검증.                                                                                                                                                                                                                                                                                           |
...
| **R12** | 특정 keyboard layout (일부 EU / 한국어 IME 활성 / dead-key layout) 에서 `?` 가 `e.key === '?'` 로 도착하지 않음          | 단축키 시트 열기 불가                                         | 본 Phase v1 = `matches: e => e.key === '?'` 단일 조건. **위험 시나리오**: ko IME 한자 변환 모드, dead-key 가 `?` 를 보조 키로 사용하는 layout. **대응**: U6 가 영문 + 한글 keyboard 양쪽 회귀 검증. 발화 안 되는 layout 발견 시 Phase 14+ 가 `e.code === 'Slash' && e.shiftKey` 보조 조건 또는 keydown→keyup 보조 매칭 도입. ShortcutSheet 는 Toolbar 의 IconButton 으로도 열 수 있어 _완전 차단은 아님_. |
...
- [x] `<PeriodicTablePanel>` 18×7 grid + ElementInfoSheet 마운트 (C1, E1) — _impl ✓ / C1·E1 phase-15_
- [x] `<CompoundBrowserPanel>` 검색 → 결과 → 추가 동작 (C2, E2) — _impl ✓ / C2·E2 phase-15_
- [x] `<ConditionsPanel>` 3 슬라이더 + 단위 토글 + reset (C3, E3) — _impl ✓ / C3·E3 phase-15_
- [x] `<MoleculeInfoPanel>` activeMolecule null/존재 + 3 Tabs (C4) — _impl ✓ / C4 phase-15_
- [x] `<ReactionResultPanel>` 4 AsyncState 분기 + products 추가 + experimental 배지 (C5, E4) — _impl ✓ / C5 ✓ / E4 phase-15_
- [x] `<ToolbarBar>` 14 슬롯 + 4 그룹 + ShortcutSheet (C6, E5, E6) — _impl ✓ / C6 ✓ / E5·E6 phase-15_
- [x] `<ToastItem>` 4 level 외양 + 자동 dismiss (C7, E7) — _impl ✓ / C7·E7 phase-15_

### 10.2 PanelRegistry 등록

- [x] `registerAllPanels()` 가 5 개 패널 등록 (C10) — `src/panels/registerAll.ts`
- [x] App.tsx 가 mount 전 1 회 호출 — `src/app/App.tsx:11`
- [x] `<SidePanelHost>` / `<ModalHost>` 가 active/is\*Open 토글 시 lazy 마운트 (phase-10 §10 정합)

### 10.3 Store retrofit

- [x] `moleculeStore.addFromMolecule(m)` 단일 entry undoable + activeId 갱신 (U4) — `src/stores/moleculeStore.ts:345` / **U4 ✓**
- [x] `moleculeStore.addFromMolecules(ms)` group undoable + 마지막 active (U5) — `src/stores/moleculeStore.ts:364` / **U5 ✓**
- [x] `selectBondMetrics(m)` standalone 순수 함수 + WeakMap memoize (U3) — `src/stores/moleculeStore.selectors.ts:66` / **U3 ✓**
- [x] retrofit 이 phase-07 selector / action barrel 에 export

### 10.4 KEY_MAP retrofit

- [x] `KeyActionId` 유니온에 `'showShortcutSheet'` 추가 — `src/viewport/interactions/shortcuts.ts:21`
- [x] `KEY_MAP` 에 `?` (Shift+/ or Slash) global scope 등록 (U6) — _impl ✓ / U6 phase-15_
- [x] phase-09 우선순위 1 (text-input gate) 적용 — input 안에서 발화 안 함

### 10.5 i18n

- [x] `panels.{ko,en}.json` 신규 — §5.4 키 트리 모두 채움
- [x] `shortcuts.{ko,en}.json` retrofit — `bondCreate.diffMolecule` + `showShortcutSheet` 한·영
- [x] `common.json#reaction.experimental{Badge,Disclaimer}` 재사용 검증 (P7)
- [x] `common.json#reaction.error.*` 재사용 + `mapReactionErrorToKey` 호출 (P8)

### 10.6 phase-10 인계 닫음

- [x] (a) `registerPanel` 5 회 호출 (D1)
- [x] (b) 13 primitives 모두 사용 (Button × 다수, IconButton × 14, Tooltip × 14, Dialog × 3, Tabs × 3, Slider × 3, Switch × 5, Spinner × 3, Input × 1, Popover × 4, Separator × 3, Label × 3, VisuallyHidden × N)
- [x] (c) `<ToolbarSlot>` 자식으로 `<ToolbarBar>` 주입
- [x] (d) `useUndoableDispatcher` 컨텍스트 사용 (Toolbar EditGroup)
- [x] (e) `<ToastContainer>` placeholder 교체 (`<ToastItem>` import)
- [x] (f) `describeKey(binding)` 사용 (Tooltip + ShortcutSheet)
- [x] (g) `KEY_MAP` 충돌 검사 (`?` 추가 시 기존 키와 충돌 없음)
- [x] (h) `is*Open` 액션 사용 (ModalHost 정합 — Toolbar 가 toggleReactionResult 등 호출)
- [x] (i) `<LoadingOverlay variant="panel">` 사용 (CompoundBrowser ResultList loading 분기)

### 10.7 phase-09 인계 닫음

- [x] §11 #5 (Q3) — Tooltip + ShortcutSheet 형식 (D-SHORTCUT-HELP)
- [x] §11 #8 (D-UNDO-TOAST) — undo 후 selection 안내 토스트 (Toolbar 클릭 경로). 단축키 경로는 §11 후속.
- [x] §11 #9 (D-BOND-DIFF-MOL-I18N) — `shortcuts.bondCreate.diffMolecule` 한·영 채움
- [x] §11 #11 (Q1) — D-LOAD-PRODUCTS, addFromMolecule(s) retrofit + fade-in 자연 발화

### 10.8 phase-10 §11 인계 닫음

- [x] §11 #1 (Q2) — `lucide-react` 확정 (D-ICONS)
- [x] §11 #2 (D-SIDEPANEL-WIDTH) — 320 px 기본값 유지 + Phase 14 후속
- [x] §11 #3 (D-TOAST-VISUAL) — `right-4 bottom-4` + level 별 외양 확정
- [x] §11 #5 (D2/D3) — Toolbar 56 px 적합성 측정 + 결론

### 10.9 라이브러리 / 빌드

- [x] `lucide-react` `package.json` 에 추가 (또는 phase-10 가 이미 추가 시 import 만 검증)
- [x] 신규 라이브러리 0 (phase-10 가 이미 등재한 lucide-react 의 사용 확정)
- [x] ESLint 가드 (§7.2) 통과 — panels 의 import 화이트리스트
- [ ] 번들 분석 — 초기 chunk 영향 +10 KB gzip 이내 (phase-14 가 실측)

### 10.10 인계 가능성

- [x] phase-12 가 본 Phase 의 `<ToolbarBar>` SMILES trigger 위치 + uiStore.toggleSmilesInput 등 추가만으로 SMILES 입력 모달 통합 가능
- [x] phase-13 가 Toolbar 의 captureBlob 헬퍼 → 정식 export 헬퍼로 _교체_ 가능 (다른 Toolbar 슬롯 수정 없음)
- [x] phase-14 가 stagger fade-in / 카테고리 트리 / 사이드패널 폭 px 정확 등 _후속_ 만 도입
- [x] phase-15 가 a11y axe + CVD 모드 팔레트 + i18n 한·영 폴리싱 _추가_ 만

### 10.11 테스트 커버리지 (v0.2 진행 정합)

- [x] **핵심 retrofit safety-net (User-선택 scope)**: U3 (selectBondMetrics) / U4 (addFromMolecule) / U5 (addFromMolecules) / C5 (ReactionResultPanel) / C6 (ToolbarBar) 작성 + green (`tests/{unit,component}/panels/`).
- [ ] U1 (units.ts) / U2 (useDebouncedQuery) / U6 (KEY_MAP) — phase-15 polish.
- [ ] C1–C4, C7–C10 — phase-15 polish.
- [ ] A1–A6 ARIA 회귀 — phase-15 polish.
...
1. **Toolbar 56 px 적합성 (D2/D3)** — 1280 px viewport 가정 시 14 슬롯 + 3 Separator = 608 px 적합. 1024 px 미달 화면은 비대상 (phase-10 R5). **잠정 답: 56 px 유지**. 사용자가 _더 많은 슬롯 추가_ 요청 시 64 px 또는 dual-row.
2. **ReactionResult 자동 열기 정책 (D-REACTION-RESULT-AUTO-OPEN)** — run() 성공 시 자동 vs 사용자 trigger. 본 Phase v1 = 사용자 trigger. **사용자 확정** 가능: settingsStore 의 `autoOpenReactionResult: boolean` 추가하면 phase 14+ 가 옵션 제공. 또는 _항상 자동 열기_ 로 확정 가능.
...
6. **테마 토글 system 모드** — 현재 Toolbar 의 테마 Switch 는 light↔dark 토글 (system 무시). settingsStore.theme = 'system' 의 _3-state_ UI (light / dark / system) 는 본 Phase 비포함 — Toolbar 가 단순 binary Switch. Phase 15 가 결정.
7. **Toast 큐 길이 50 (P12) UI 표시** — 50 도달 시 가장 오래된 자동 dismiss (phase-07 R7). UI 는 _모든_ 항목 렌더 → 화면 영역 초과. 본 Phase v1 = 모두 렌더 (50 도달은 비현실적 가정). Phase 14+ 가 max-height + 스크롤 도입.
...
11. **`addFromMolecule` 호출자 (Toolbar 외)** — 본 Phase 는 ReactionResult 패널만 호출. Phase 12 의 SMILES 입력은 `addFromSmiles` 사용. Phase 13 의 JSON import 는 별도 진입점. 향후 _별도 import 경로_ 가 추가되면 본 retrofit 의 활용 범위 확장.
...
13. **`<RuleTab>` 의 rule 메타 노출 (v1 비표시 — 후속)** — v1 은 `appliedRuleId` 텍스트만 표시 (CD5, `ReactionResult` 계약에 source/version/license 없음). 출처 메타 노출이 필요해지면 Phase 06 이 `ReactionResult` 에 rule 메타를 additive 포함하거나 rule manifest store 를 추가 — 그 결정 전까지 UI 는 source 행을 그리지 않는다 (계약에 없는 데이터 광고 금지).
...
| **15 (Polish)**              | (a) **a11y axe 자동 검증** — PeriodicGrid / Tabs / Slider / Dialog / Toast 모두. (b) **CVD 모드 실 팔레트** — ExperimentalBadge 의 `*` 마커 + ThermoFlag 아이콘 + 색상 대비 검증. (c) **i18n 한·영 최종 폴리싱** — panels.json 모든 키 자연스러운 표현 + chemistry 용어 사전 일관성. (d) **PeriodicGrid axe 자동 검증** — 화살표 키 nav 는 본 Phase 11 §6.1 에서 이미 구현 (roving tabindex); Phase 15 는 axe 회귀 검증만. (e) **테마 system 모드 UI** (§11 #6). (f) **Toast 큐 50 도달 UI** (§11 #7). (g) **clipboard fallback** (§11 #14). (h) **i18n 부팅 안전 시뮬레이션** — panels 패널 내부 t() 가 missing key 일 때 폴백 (key 자체 표시).                               |

### 12.1 소급 수정

- **phase-07 (Stores)**: `moleculeStore.actions` 에 `addFromMolecule` / `addFromMolecules` 추가. `moleculeStore.selectors` 에 `selectBondMetrics` + 타입 (`BondMetric`, `BondAngleMetric`, `BondMetricsResult`) 추가. **추가 retrofit (본 Phase 가 요구)**: ① `moleculeStore` 에 `lastUndoSeq: number` state 필드 (`undo` 시 +1, `redo`/일반 변경 불변 — D-UNDO-TOAST §6.13 단일 관찰자). ② `uiStore` 에 `selectSelection` (→ `{ atomIds: readonly string[]; bondIds: readonly string[] }`) + `selectHasSelection` (→ boolean) selector. ③ 시그니처 확정: `addFromCompound(cid: number, opts?): Promise<Result<MoleculeId, IngestError>>`, `selectCompoundSearch` 의 `CompoundSearchSlice` 형태(§3.1 07행) 동결. barrel `src/stores/index.ts` 갱신.
...
- **phase-10 (UI Framework)**: `<ToastContainer>` 의 placeholder `<ToastSlot>` 을 `<ToastItem>` (`@/panels` import) 로 교체. App.tsx 가 `registerAllPanels()` 호출 + `<ToolbarBar/>` 주입 + `<IoDialog/>` 직접 마운트(phase-13). **추가 retrofit (본 Phase 가 요구)**: `installAppShortcuts` 시그니처를 `installAppShortcuts({ dispatcher, onAction }): () => void` 로 확장 — `onAction(actionId: KeyActionId)` 콜백이 KEY_MAP 의 비-undo 바인딩(`showShortcutSheet` 등)을 통지 (D-SHORTCUT-OPEN-STATE 단일 디스패치 경로). App.tsx 가 `onAction` 주입.
...
**없음**. 본 Phase 가 architecture 의 §1.3 ⑥ "결합 변화 애니메이션" 약속을 _경로 닫음_ 으로 이행 — 약속 자체는 이미 존재. §6 의 "UI Panels — 주기율표, 화합물 검색, 조건, 분자 정보, 반응 결과, Toolbar" 명세도 본 Phase 가 충실히 이행.

---

_문서 버전: 0.2 (구현 정합)_
_작성일: 2026-04-27 (v0.1 초안) / 2026-05-18 (v0.2 — phase-09/10 v0.2 정합)_

> **v0.2 변경 요약 (forward 정합 — 코드 미변경, phase-11 미구현):**
>
> 1. `useUndoStack()` import 경로 `@/viewport` → **`@/stores`** 배럴 (phase-09 v0.2 가 undo 코어를 stores 레이어로 이전; KEY_MAP/describeKey 는 `@/viewport` 유지). EditGroup import + 인계표 09 행 정정.
> 2. moleculeStore retrofit (`addFromMolecule`/`addFromMolecules`) 의 dispatcher swap 식별자 = `dispatcher` (phase-09 v0.2, 초안 §1943 와 일치 — 변경 없음 확인).
> 3. lucide-react 는 본 Phase(Toolbar 아이콘)가 첫 소비자 — phase-10 미사용 deferral 정합.
>
> phase-09/10 코드 무모순. phase-11 구현 시 본 v0.2 spec 대로 작성.
